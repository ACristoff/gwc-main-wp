...
/**
 * 
 * Image & Text Column Block Template.
 *
...
$id = 'ql-image-text-column-' . $block['id'];
if( !empty($block['anchor']) ) {
    $id = $block['anchor'];
}

// Create class attribute allowing for custom "className" value.
$className = 'acf-block ql-image-text-column';
...
$template = array(
	array('core/heading', array(
		'level' => 3,
		'content' => 'Full-width two-column block example',
	)),
...
if( !empty( $block['data']['_is_preview'] ) ):
    echo '<figure style="margin:0;">';
        echo '<img src="'. get_stylesheet_directory_uri() .'/assets/block-previews/block-preview-image-text-column.jpg" alt="Preview of the Image & Text Column Block.">';
    echo '</figure>';
else:

echo '<div id="' . esc_attr($id) . '" class="align-full align-full-limited no-pad ' . esc_attr($className) .'" style="'.$marginTop.$marginBottom.'">';
